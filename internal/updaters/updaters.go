...
type Definition struct {
	...
	// chocolatey; v2 picks up the hold-based managers below).
	// Custom definitions trust the author — the SPA can't introspect
	// the playbook, so the operator opts in implicitly by appearing in
	// the exclusion form's dropdown.
	SupportsExclusions bool
	// UsesHolds marks managers whose exclusion mechanism flips a
	// persistent host-side flag (apt-mark hold, brew pin, snap refresh
	// --hold, flatpak mask, pkgin/xbps/scoop). The runner reads from
	// holds.Store before such applies and writes back after a
	// successful run so removing an exclusion row clears the matching
	// host-side hold on the next Apply.
	UsesHolds bool
